...
// This method assigns a core ID to a job, which marks it as "active" in the
// simulator.  All active jobs have non-negative job_id's.
int set_active_job(int job_id, int core_id, simulator_job_list_t *jobs, int active_jobs)
{
	...
	{
		// If the job's id matches the one passed in...
		// and the job.arrived is non-zero (it has arrived)
		if (jobs[i].job_id == job_id && jobs[i].arrived)
		{
			// Set the job's core id to the one passed in
...
// Run through all known jobs, find the ones that have arrived, and print the
// job id.
	...
	while (active_jobs > 0)
	{
		...
		for (i = 0; i < active_jobs; i++)
		{
			if (jobs[i].run_time == 0)
			{
				...
				// This call merits a comment, since the
				// method names make discerning the behavior
				// difficult.  It relies on specialized
				// behavior and makes no mention of it, which
				// makes for unreadable code and wastes
				// developers' time.
				//
				// scheduler_job_finished will prompt the
				// scheduler to destroy the given job, and
				// return the ID of the next known job (on the
				// scheduler side) which should be scheduled
				// on THIS core.  If -1 is returned, then the
				// core is deemed idle.
				...
				if (i != active_jobs - 1)
					// Whomever wrtoe this should be
					// fired.  Modifying a list in place
					// is tricky, doing so without
					// comments is stupid.
				...
				// If the core is not idle, but the returned
				// id doesn't match the local records, throw
				// an error.
				...
				if (quantum_clock[i] == 0)
				{
					for (j = 0; j < active_jobs; j++)
					{
						if (jobs[j].core_id == i)
						{
							...
							// It's as if this person wants to piss off everyone who has to maintain their code.
							// Returns -1 if the core shoudl remain idle, otherwise the job id of the next
							// job for this core.
							int new_job_id = scheduler_quantum_expired(jobs[j].core_id, time);

							// Set this job as "Inactive"
							jobs[j].core_id = -1;

							// Reset quantum
							quantum_clock[core_id] = quantum;

							// If the core is not idle, but the job given is not assigned to the
							// core, throw an error
							if ( new_job_id != -1 && !set_active_job(new_job_id, core_id, jobs, active_jobs) )
							{
		...
		for (i = 0; i < active_jobs; i++)
		{
			if (jobs[i].core_id != -1)
			{
				...
				// REALLY?!?  MAGIC NUMBERS?!?  Fire this person.
				if (jobs[i].job_id < 10)
